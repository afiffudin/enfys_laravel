...
class SerahTerimaController extends Controller
{
    ...
    {
        $jadwalData['data'] = Jadwal::orderby("Tanggal_keberangkatan", "asc")
            ->select('id_inventaris', 'Tanggal_keberangkatan')
            ->get();
        return view('TambahSerah')->with("jadwalData", $jadwalData);
    }
    public function getserah_terima_inventaris($Inventaris_mobilid = 0)
    {
        //fetch jadwal by Inventaris_mobilid
        $empData['data'] = Jadwal::orderby("Tanggal_keberangkatan", "asc")
            ->select('id_inventaris', 'Tanggal_keberangkatan')
            ->where('Inventaris_mobil', $Inventaris_mobilid)
            ->get();
    ...
    {
        DB::table('serah_terima_inventaris')->where('id_inventaris', $id_inventaris)->delete();
        return back();
    }
